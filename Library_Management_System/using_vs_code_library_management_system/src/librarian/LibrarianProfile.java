...
public class LibrarianProfile extends JFrame {
    ...
    public LibrarianProfile(String s, Connection con, String name, String user) {
        super(s);
        // setSize(1080,780);
    ...
    public void get_profile_date() {
        if (con != null && EstablishConection.getEstblishConnection() != null) {
            try {
                String q = "SELECT name,password,gender,email_id,phone_no,security_question FROM librarian WHERE username=?";
        ...
        public void actionPerformed(ActionEvent e) {
            if (con != null && EstablishConection.getEstblishConnection() != null) {
                if (e.getSource() == logout) {
                    ...
                    dispose();
                    new LibrarianControl("Librarian Control", con, name, user).setVisible(true);
                    ...
                    if (name1.isEmpty()) {
                        ...
                        try {
                            String q = "SELECT name FROM librarian WHERE username=? && security_question=? && answer=?";
                            PreparedStatement ps = con.prepareStatement(q);
                            ps.setString(1, user);
                            ...
                            ResultSet rs = ps.executeQuery();
                            if (rs.next()) {
                                q = "UPDATE librarian SET name=?,password=?,email_id=?,phone_no=? WHERE username=?";
